...
function handleExportCsv() {
  const bom = '\uFEFF'
  const header = 'Produit;Fournisseur;Quantité'
  const rows = lines.value.map(
    (line) => `${line.productName};${line.supplier};${line.totalQuantity}`,
  )
  ...
  a.href = url
  a.download = `bon-fournisseur-${todayISO.value}.csv`
  a.click()
...
async function loadData() {
  ...
  try {
    ...
    if (firstVente) {
      lines.value = await fetchSupplierOrder(firstVente.id)
    }
  } catch {
